...
describe('speech dev App', () => {
  it('显示通用运行时、加速器和实际模型精度，不暴露后端切换控件', async () => {
    const mlxSnapshot = labSnapshot();
    mlxSnapshot.tts.initialization = {
      ...mlxSnapshot.tts.initialization!,
      runtime: 'mlx',
      accelerator: 'metal',
      device: 'gpu:0',
    ...
    mlxSnapshot.asr.initialization = {
      ...mlxSnapshot.asr.initialization!,
      runtime: 'mlx',
      accelerator: 'metal',
      device: 'gpu:0',
    ...
    api.getSnapshot = vi.fn(async () => mlxSnapshot);
    window.speechLab = api;

    const wrapper = await mountApp();

    expect(wrapper.text()).toContain('MLX / Metal / BF16');
    expect(wrapper.text()).toContain('模型驻留策略');
    expect(wrapper.text()).toContain('gpu:0 · bfloat16 / tokenizer float32');
    expect(wrapper.find('[data-test="runtime-backend"]').exists()).toBe(false);
  });

  it('未初始化时按默认运行时显示配套加速器', async () => {
    api.getSnapshot = vi.fn(async () => labSnapshot({
      tts: {
        ...labSnapshot().tts,
        state: 'uninitialized',
        initialized: false,
