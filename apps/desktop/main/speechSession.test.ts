...
describe('speechSession', () => {
  ...
  it('一次 SDK 批量推理按批次耗时计算行级与整体 CPM', async () => {
    const clock = new FakeClock();
    const provider = new FakeProvider();
  ...
  it('首次自动初始化失败只记录错误，不自行循环重试', async () => {
    ...
    provider.onInitialize = async () => {
      throw new ASRProviderError('MPS_UNAVAILABLE', 'MPS unavailable');
    };
    const session = new SpeechSession({ providerFactory: () => provider });

    await expect(session.initialize('automatic')).rejects.toMatchObject({ code: 'MPS_UNAVAILABLE' });
    await flushPromises();

    expect(provider.initializeCalls).toBe(1);
    expect(session.snapshot()).toMatchObject({
      state: 'failed',
      lastError: { code: 'MPS_UNAVAILABLE' },
      idleDeadlineMs: null,
    });
  });
});
