...
import { ASR_PROVIDER_ERROR_CODES, ASRProviderError } from '@electron-qwen-speech/application';
...
export const QWEN_LOCAL_ASR_DEFAULTS = {
  pythonExecutable: path.join(REPOSITORY_ROOT, '.venv', 'bin', 'python'),
...
function parseInitializeResult(value: unknown): ASRInitializeResult {
  ...
  return {
    ...
    asrDevice: readString(value, 'asrDevice'),
    alignerDevice: readString(value, 'alignerDevice'),
    asrDtype: readString(value, 'asrDtype'),
    alignerDtype: readString(value, 'alignerDtype'),
    loadCount: readNonNegativeInteger(value, 'loadCount'),
  };
}
...
export class QwenLocalASRProvider implements ASRBatchProvider {
  ...
  async initialize(): Promise<ASRInitializeResult> {
    ...
    if (
      path.resolve(result.asrModelPath) !== this.#asrModelPath
      || path.resolve(result.alignerModelPath) !== this.#alignerModelPath
    ) {
      throw new ASRProviderError('INITIALIZATION_FAILED', 'ASR Sidecar 返回的模型路径与配置不一致。', {
        details: {
          expected: {
            asrModelPath: this.#asrModelPath,
            alignerModelPath: this.#alignerModelPath,
          },
          actual: {
            asrModelPath: result.asrModelPath,
            alignerModelPath: result.alignerModelPath,
          },
        },
      });
    }
