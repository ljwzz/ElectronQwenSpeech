...
async function readAudioDurationSeconds(audioPath: string): Promise<number> {
...
async function assertValidTimestamps(
  result: ASRTranscriptionResult | ASRBatchTranscriptionItemResult,
  audioPath: string,
): Promise<void> {
  ...
  const duration = await readAudioDurationSeconds(audioPath);
  const tolerance = Math.max(0.5, duration * 0.05);
  expect(result.timestamps.at(-1)!.end).toBeLessThanOrEqual(duration + tolerance);
}
...
async function loadASRFixtureExpectation(expectationPath: string): Promise<ASRFixtureExpectation> {
  ...
  if (
    ...
    || ('language' in value && (typeof value.language !== 'string' || value.language.length === 0))
  ) {
...
describe.sequential('QwenLocalASRProvider real MLX integration', () => {
  const stderr: string[] = [];
  ...
  it('在 Metal gpu:0 初始化固定 MLX ASR 与 Forced Aligner', async () => {
    ...
    expect(initialization).toMatchObject({
      ...
      asrModelPath: process.env.ELECTRON_QWEN_SPEECH_ASR_MODEL_PATH,
      alignerModelPath: process.env.ELECTRON_QWEN_SPEECH_ALIGNER_MODEL_PATH,
      asrModelRevision: expected.asrModelRevision,
      alignerModelRevision: expected.alignerModelRevision,
      asrModelPrecision: expected.asrModelPrecision,
      alignerModelPrecision: expected.alignerModelPrecision,
      loadCount: 1,
    });
  }, 600_000);
  ...
  it.each(ASR_FIXTURE_CASES)(
    ...
    async ({ taskId, audioPath, expectationPath }) => {
      ...
      const result = await provider.transcribe({
        taskId,
        audioPath,
        ...(expectation.language === undefined ? {} : { language: expectation.language }),
      });
      ...
      await assertValidTimestamps(result, audioPath);
    },
    300_000,
  );

  it('通过批量 API 逐项转写两个 fixture 并保持输入顺序', async () => {
    ...
    for (const [index, item] of result.items.entries()) {
      ...
      await assertValidTimestamps(item, fixtureCases[index]!.audioPath);
    }
  }, 300_000);
