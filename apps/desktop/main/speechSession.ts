...
export class SpeechSession {
      ...
      try {
        if (this.#initialization) {
          ...
          if (this.#provider !== provider || this.#stopping)
            throw new ASRProviderError('PROVIDER_DISPOSED', '状态确认期间 Provider 已停用。');
          ...
          if (fixtureGroup.length === 1) {
            ...
            for (const [index, batchResult] of result.items.entries()) {
              this.#applyResult(itemGroup[index]!, {
                ...
                alignmentStatus: batchResult.alignmentStatus,
  ...
  #applyResult(
    ...
    item.alignmentStatus = result.alignmentStatus;
