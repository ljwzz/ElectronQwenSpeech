...
export const QWEN_LOCAL_TTS_DEFAULTS = {
  ...
  pythonExecutable: path.join(REPOSITORY_ROOT, 'services', 'tts-sidecar', '.venv-mlx', 'bin', 'python'),
  accelerator: 'metal',
  device: 'gpu:0',
  modelFormat: 'mlx' as SpeechModelFormat,
  modelRevision: '52f4770fd9726457eae3d3b6aa92047a25a10776',
  modelPrecision: 'bfloat16' as SpeechModelPrecision,
  speechTokenizerPrecision: 'float32' as SpeechModelPrecision,
...
function readNullableString(record: Record<string, unknown>, key: string): string | null {
  ...
  if (value !== null && typeof value !== 'string')
    throw protocolError(`TTS Sidecar 响应字段 ${key} 必须是字符串或 null。`, record);
  return value;
}
...
function readRuntime(record: Record<string, unknown>): SpeechRuntime {
  ...
  if (runtime !== 'mlx')
    throw protocolError(`TTS Sidecar 返回未知运行时：${runtime}`, record);
  return 'mlx';
}
...
function readModelFormat(record: Record<string, unknown>): SpeechModelFormat {
  ...
  if (modelFormat !== 'mlx')
    throw protocolError(`TTS Sidecar 返回未知模型格式：${modelFormat}`, record);
  return 'mlx';
}
...
function readModelPrecision(record: Record<string, unknown>, key: string): SpeechModelPrecision {
  ...
  if (!MODEL_PRECISION_SET.has(precision))
    throw protocolError(`TTS Sidecar 返回未知模型精度：${precision}`, record);
  return precision;
}
...
function parseInitializeResult(value: unknown): TTSInitializeResult {
  ...
  return {
    ...
    accelerator: readString(value, 'accelerator') as TTSInitializeResult['accelerator'],
    modelPath: readString(value, 'modelPath'),
    device: readString(value, 'device') as TTSInitializeResult['device'],
    modelFormat: readModelFormat(value),
    modelRevision: readNullableString(value, 'modelRevision') as TTSInitializeResult['modelRevision'],
    modelPrecision: readModelPrecision(value, 'modelPrecision') as TTSInitializeResult['modelPrecision'],
    speechTokenizerPrecision: readModelPrecision(value, 'speechTokenizerPrecision') as TTSInitializeResult['speechTokenizerPrecision'],
    dtype: readString(value, 'dtype') as TTSInitializeResult['dtype'],
...
function assertRuntimeMetadata(result: TTSInitializeResult, modelPath: string): void {
  ...
  if (
    ...
    throw new TTSProviderError('RUNTIME_DEVICE_MISMATCH', 'TTS Sidecar 运行时或设备与请求不一致。', {
      details: { expected, actual: result },
    });
  }
  if (
    path.resolve(result.modelPath) !== modelPath
    || result.modelRevision !== expected.modelRevision
    || result.modelPrecision !== expected.modelPrecision
    || result.speechTokenizerPrecision !== expected.speechTokenizerPrecision
    || result.dtype !== result.modelPrecision
  ) {
    throw new TTSProviderError('MODEL_INTEGRITY_FAILED', 'TTS Sidecar 模型身份或精度与固定配置不一致。', {
      details: { expected, actual: result },
    });
  }
}
...
export class QwenLocalTTSProvider implements TTSProvider {
  ...
  async initialize(): Promise<TTSInitializeResult> {
    ...
    assertRuntimeMetadata(result, this.#modelPath);
    return result;
  }
